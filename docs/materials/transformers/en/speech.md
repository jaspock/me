...
HuBERT is a self-supervised model designed to process speech by transforming audio waveforms into discrete representations, which can be roughly thought of as an equivalent to tokens in text processing. Later works have shown that these representations can be integrated into LLMs so that they work as multimodal text/speech models.

HuBERT is obtained via an iterative method that alternates two steps: clustering and prediction. The clustering step ($k$-means into 500 pseudo-tokens) assigns each audio frame to one of several clusters. In the first step, frames are represented with vectors resulting from the Mel spectrogram. In the remaining steps, the representations are obtained from the intermediate layers of the HuBERT model pre-trained in the previous iteration. Similarly to Wav2vec 2.0, the prediction step consists of predicting the masked parts of the input using the cluster indexes as the output vocabulary. Lastly, the pre-trained HuBERT model (after removal of the prediction layer) can fine-tuned to perform specific tasks, such as speech recognition using the CTC loss function. As we will see later, the discrete representations are also interesting on their own.

The following figure, taken from the paper "[HuBERT: Self-Supervised Speech Representation Learning by Masked Prediction of Hidden Units](https://arxiv.org/abs/2106.07447)," shows the model's architecture:
...
Discrete speech phonetic units as those generated by HuBERT can be incorporated into LLMs to create the so-called SpeechLM models, which are large language models (LLMs) that can process spoken and written language in both their input and output. Spirit LM is an example of this: a Llama-2 LLM fine-tuned with a mixture of text and speech, encoded either as the original BPE tokens or the (deduplicated) phonetic units generated by HuBERT, respectively, with a special token preprended to each type of input. The model is fine-tuned in the usual next-token prediction task. A speech-to-text system conditioned on the HuBERT speech tokens is added for speech generation. Optionally, sentiment information in the form of pitch and style tokens can be added to the training data so that the model does not generate plain speech. 

This image from the paper "[Spirit LM: Interleaved Spoken and Written Language Model](https://arxiv.org/abs/2402.05755)" illustrates the model's architecture:

![](../assets/imgs/spiritlm.png)

Even though the use of the speech pseudo-tokens is an interesting approach, when the purpose is not to generate speech with the LLM, other researchers have shown that directly using the continuous representations learned by text-to-speech models like Whisper with a bridge projection layer before the LLM input can also be effective as shown in this figure from "[An Embarrassingly Simple Approach for LLM with Strong ASR Capacity](https://arxiv.org/abs/2402.08846)":
