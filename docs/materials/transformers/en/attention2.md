...
Additionally, depending on the task, language-specific tokens can be used as the first token to indicate the language of the text. This practice is more common in encoder-based models (e.g., for named entity recognition or sentence classification) than in decoder-like models. These special tokens are added to the vocabulary, prepended to every sentence during both training and inference, and learned in the same way as the *MASK* token in BERT-like pretraining.
...
## The Different Faces of Attention
...
2. An attention matrix represents the level of attention given to embeddings of each token from the previous layer (columns) when computing the embedding of the token for a specific row in a transformer encoder's attention head. A darker color indicates
